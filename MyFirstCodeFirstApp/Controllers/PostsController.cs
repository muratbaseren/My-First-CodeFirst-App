...
namespace MyFirstCodeFirstApp.Controllers
{
    ...
    {
        private MyBlogDBEntities db = new MyBlogDBEntities();
        ...
        {
            Post db_post = db.Posts.FirstOrDefault(x => x.Id == post.Id);

            if (db_post.IsDraft == true && post.IsDraft == false)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                db_post.IsDraft = post.IsDraft;
                db_post.ModifiedDate = DateTime.Now;
                db_post.CategoryId = post.CategoryId;
